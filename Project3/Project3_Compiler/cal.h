...
void  initCalcFile(){
    CalcF = fopen("./Calculator/CalcF.txt", "w");
    fclose(CalcF);
}

void clearClacFile(){
    char clearCMD[50], clearCMD2[50];
    strcpy( clearCMD, "rm -f ./Calculator/CalcF.txt" );
	system(clearCMD);
	strcpy( clearCMD2, "rm -f ./Calculator/Results/CalcF.txt_results.txt" );
	system(clearCMD2);
}
...
void addCalcFile(){
    ...
    CalcF = fopen("./Calculator/CalcF.txt", "a");
    for (int i=0; i<len; i++){
        numtemp = NumArr[i];
        optemp = OpArr[i];
